...
export function SettingsView({ sessionCount }: SettingsViewProps) {
  ...
  // Demande l'autorisation au système d'exploitation
  ...
  const triggerLocalNotification = (title: string, body: string) => {
    if (Notification.permission === 'granted') {
      new Notification(title, {
        body,
        icon: '/chess-icon.png' // Ajoutez une icône dans public/ si vous voulez
      })
    }
  }
  ...
  const toggleScheduling = (active: boolean) => {
    ...
    if (active) {
      // Simule un planificateur quotidien côté client simple (vérifie toutes les heures si l'onglet dort)
      const intervalId = setInterval(() => {
        const hours = new Date().getHours()
        // Déclenche à 10h du matin par exemple si des chapitres attendent
